...
class RidesStateless extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (rides.length == 0) {
      ...
      return _mainPage(context, rides);
    }
  }
}
...
class _RidesState extends State<Rides> {
  ...
  Future<List<Ride>> _fetchRides(String id) async {
    ...
    final response = await http.get(AppConfig.of(context).baseUrl +
        '/rides?date=${dateFormat.format(now)}&driver=${Provider.of<AuthProvider>(context).id}');
            ...
            Expanded(
                child: FutureBuilder<List<Ride>>(
                    ...
                    builder: (context, snapshot) {
                      if (snapshot.hasData) {
                        return RidesStateless(rides: snapshot.data);
